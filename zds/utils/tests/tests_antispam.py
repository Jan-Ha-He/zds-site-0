...
class SpamDetectorTestCase(TestCase):
    ...
    def test_check_profile_no_bio(self, mock_send_alert):
        ...
        with self.assertLogs("zds.utils.spam_detector", level="INFO") as log:
    ...
    @patch("zds.utils.spam_detector.SpamDetector.check", return_value=0)
    ...
    @patch("zds.utils.spam_detector.SpamDetector.send_alert")
    @patch("zds.utils.spam_detector.SpamDetector.check", return_value=1)
